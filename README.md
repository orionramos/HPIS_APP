...
HPIS_APP es una aplicación de Realidad Virtual desarrollada en Unity para la interacción y posicionamiento dinámico de interfaces y animaciones 3D en un entorno inmersivo. Permite a los usuarios ubicar un canvas de feedback y un anchor de animaciones en el espacio, utilizando los controles de Meta Quest para una experiencia intuitiva y flexible. El sistema está diseñado para facilitar la retroalimentación visual, auditiva y animada en actividades educativas o de simulación.

---

## Guía de Controles VR 🎯

### 🖐️ Control Derecho

#### 🕹️ Joystick Derecho
- **Modo Normal** (sin gatillo de mano presionado):
  - `X` (Izquierda/Derecha): Mueve el anchor en el eje X local
  - `Y` (Arriba/Abajo): Mueve el anchor adelante/atrás
- **Modo Escala** (con gatillo de mano presionado):
  - `X` (Izquierda/Derecha): Controla la escala del anchor

#### 🎮 Botones
- `A`: Ancla el canvas (solo cuando no está anclado)
- `B`: Resetea el canvas y reinicia el proceso
- `Gatillo índice`: Fija el anchor de animaciones en su posición
- `Gatillo de mano`: Mantener presionado para cambiar a modo escala con el joystick

### 🖐️ Control Izquierdo

#### 🕹️ Joystick Izquierdo
- `X` (Izquierda/Derecha): Rota el anchor en Y (giro horizontal)
- `Y` (Arriba/Abajo): Rota el anchor en X (inclinación vertical)
...
### 1️⃣ Canvas
1. Posicionar el canvas con el controlador
2. Presionar `A` para anclarlo

### 2️⃣ Animaciones
1. El anchor de animaciones aparece automáticamente después de anclar el canvas
2. Usar joystick derecho para mover en el espacio
3. Usar joystick izquierdo para rotar
4. Mantener grip + joystick derecho X para escalar
5. Presionar gatillo índice para fijar el anchor
...
| Control                  | Función                                 |
|--------------------------|-----------------------------------------|
| 🕹️ Joystick Derecho      | Mover anchor/Escalar (con grip)         |
| 🕹️ Joystick Izquierdo    | Rotar anchor                            |
| 🎮 Botón A               | Anclar canvas                           |
| 🎮 Botón B               | Resetear                                |
| 🔫 Gatillo índice        | Fijar anchor de animaciones             |
| ✊ Gatillo mano           | Activar modo escala                     |
